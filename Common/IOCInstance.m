    ...
    methods
        ...
        function featureVals = calcFeatures(obj, state, control)
            ...
            if obj.basisFeatureFlags.dddq
                ...
                basisFeatureVals.dddq = obj.numDiff(basisFeatureVals.ddq, minWinLen, lenTime, lenWidth);
            end
            
            if obj.basisFeatureFlags.dddx
                minWinLen = 1;
                lenWidth = lenFullFrame;
                basisFeatureVals.dddx = obj.numDiff(basisFeatureVals.ddx, minWinLen, lenTime, lenWidth);
            end
            
            if obj.basisFeatureFlags.dtau
                minWinLen = 1;
                lenWidth = lenFullJoint;
                basisFeatureVals.dtau = obj.numDiff(basisFeatureVals.tau, minWinLen, lenTime, lenWidth);
            end
            
            if obj.basisFeatureFlags.ddtau
                minWinLen = 2;
                lenWidth = lenFullJoint;
                basisFeatureVals.ddtau = obj.numDiff(basisFeatureVals.dtau, minWinLen, lenTime, lenWidth);
            end
