...
%                 [mdl_lie, trcMod_lie, initPos_lie] = createJumpModel_Lie(trc, 1, initPosWindow, S2sc, calibPose, EKFCodePath);
                [mdl_shldr, trcMod_eul, initPos] = createJumpModel_shldr(trc, 1, initPosWindow, S2sc, calibPose, EKFCodePath);
...
%                     vis.addModel(mdl_lie);
                ...
                mdl_shldr.position = mdl_eul.position;
                mdl_shldr.position(2) = mdl_shldr.position(2) + 1; % offset model position 1 meter
                mdl_shldr.forwardPosition;
...
%                     jointAngles_lie = getLieModelJAs(mdl_lie);
                    ...
                    mdl_shldr.position = mdl_eul.position;
                    mdl_shldr.position(2) = mdl_shldr.position(2) + 1; % offset model position 1 meter
                    mdl_shldr.position(10:12) = rshldrRPY(i,:);
                    mdl_shldr.position(15:17) = lshldrRPY(i,:);
                    mdl_shldr.forwardPosition;
