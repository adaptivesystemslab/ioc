% Calculates marker MAE using models calculated from different window sizes
...
%% After running grid search below (41 hours), optimal window is [250,450]
windowStart = [100, 150, 200, 250];
windowSize = [50, 100, 150, 200, 250, 300]; 


markerMAE_all_ave = zeros(numel(windowStart), numel(windowSize), numel(partsToRun)); % MAE for all markers, averaged over all time, for selected jump
% markerMAE_marker_ave = zeros(numel(processNoise), numel(observationNoise), numel(partsToRun), 30); % MAE for each marker, averaged over all time, for selected jump
...
for i_part = 1:numel(partsToRun)
    ...
    for i_winStart = 1:numel(windowStart)
        for i_winSize = 1:numel(windowSize)
%             for EKF_cov = 1:numel(covariance)
                ...
                %                 trc = fillInTRCFrames(trc);
                                ...
                                initPosWindow = [windowStart(i_winStart), windowStart(i_winStart)+windowSize(i_winSize)];
...
%                                 ekf_eul = EKF_Q_DQ_DDQ(mdl_eul);
                ...
                %                 ekf_lie.process_noise = P;
                                ekf_eul.process_noise = eye(ekf_eul.sizeX) * 1.5;
...
%                                 EKF_runtime = toc
                                ...
                                markerMAE_all_ave(i_winStart,i_winSize,i_part) = mean(mae_eul_FB);
