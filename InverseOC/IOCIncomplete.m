...
iocResultsSaver=struct;
iocResultsSaver.iocConfig=trialInfo.runName;
iocResultsSaver.modelType=trialInfo.modelType;
iocResultsSaver.baseModel=trialInfo.baseModel;
iocResultsSaver.model=trialInfo.model;
iocResultsSaver.origDataSubPath=trialInfo.subpath;
iocResultsSaver.dt=dt;
iocResultsSaver.candidateFeatures=trialInfo.candidateFeatures;
...
%     drawnow
    ...
    %save data
    iocResultsSaver.iocResults(k).trajSegIndex=segFrame;
    iocResultsSaver.iocResults(k).initialTime=segTraj.trajT(1);
    iocResultsSaver.iocResults(k).initialJointAngle=segTraj.q(1,:);
    iocResultsSaver.iocResults(k).initialJointVelocity=segTraj.dq(1,:);
    iocResultsSaver.iocResults(k).finalTime=segTraj.trajT(end);
    iocResultsSaver.iocResults(k).finalJointAngle=segTraj.q(end,:);
    iocResultsSaver.iocResults(k).finalJointVelocity=segTraj.dq(end,:);
    iocResultsSaver.iocResults(k).weights=weightsVec(end,:);
    fprintf("Press any key to proceed the next segment\n");
%     pause;
end

fprintf("Now.....Saving Data\n");
pause(1)
savePath=fullfile(savePath,trialInfo.runName);
save(savePath,'iocResultsSaver')
fprintf("Saving data completed!\n");
