function [model, trc, initPos, modelLinks] = createJumpModel_setLinkLengths_inertia(trc, attachMarkers, crop, S2sc, calibPose, EKFCodePath, partNum)
...
model = rlCModel('JumpModel_Eul_inertia.xml');
...
if(visu)
    ...
    for i = 1:length(model.bodies)
        vis.addMarker(['COM_' model.bodies(i).name], ...
            model.bodies(i).t(1:3, 4) + model.bodies(i).com, [0.8 0 0 1]);
    end
end
