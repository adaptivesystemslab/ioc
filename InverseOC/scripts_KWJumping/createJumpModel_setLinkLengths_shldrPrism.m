function [model, trc, initPos, modelLinks] = createJumpModel_setLinkLengths_shldrPrism(trc, attachMarkers, crop, S2sc, calibPose, EKFCodePath, partNum)
...
model = rlCModel('JumpModel_Eul_shldrPrism.xml');
...
if(calibPose=='T')
    ...
    initPos(12) = atan2(-(relbow(2)-rshoulder(2)),-(relbow(3)-rshoulder(3))); %shoulder abduction
    initPos(13) = -pi/4; %45 deg shoulder internal rotation, since ppl have tendency to do this during T-pose
    initPos(14) = deg2rad(5); %put small bend in elbow in correct direction
    initPos(15) = -pi/2; %assume forearm supinated 90 degrees (palm facing down in T-pose)
    initPos(18) = atan2(lelbow(2)-lshoulder(2),-(lelbow(3)-lshoulder(3))); %shoulder abduction
    initPos(19) = -pi/4; %45 deg shoulder internal rotation, since ppl have tendency to do this during T-pose
    initPos(20) = deg2rad(5); %put small bend in elbow in correct direction
    initPos(21) = -pi/2; %assume forearm supinated 90 degrees (palm facing down in T-pose)
    initPos(25) = deg2rad(-5); %put small bend in knee in correct direction
    initPos(32) = deg2rad(-5); %put small bend in knee in correct direction
elseif(calibPose=='L')
    initPos(11) = pi/2; %shoulder elevation to 90 degrees
    initPos(14) = pi/2; %elbow flexed to 90 degrees
    initPos(15) = -pi/2; %forearm supinated 90 degrees (palms facing in L-pose)
    initPos(17) = pi/2; %shoulder elevation to 90 degrees
    initPos(20) = pi/2; %elbow flexed to 90 degrees
    initPos(21) = -pi/2; %forearm supinated 90 degrees (palms facing in L-pose)
end
...
trNum = find(ismember(transformNames,'lprism2lshoulder')==1);
T = model.transforms(trNum).t;
FT = model.transforms(trNum).frame_in.t;
...
trNum = find(ismember(transformNames,'rprism2rshoulder')==1);
T = model.transforms(trNum).t;
FT = model.transforms(trNum).frame_in.t;
...
%             end
    ...
    if(visu)
        ...
        for i = 1:numel(Markers)
            m_pos = mean(trc.data.(markerNames{i+2})(crop1:crop2,:));
            vis.addMarker(num2str((markerNames{i+2})),m_pos);
            vis.update();
        end
