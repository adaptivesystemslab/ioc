...
plotJointAngles = 0; % DOESN'T WORK WITH 2D MODEL
...
%     JA.shiftFlipData_lie = zeros(36,33);
    ...
    % This is set positive if 2D jump representation is not good (shoulder
    % elevation or elbow flexion moves too quickly due to shoulder abduction
    JA.bad2D = zeros(12,3);
    ...
    %                 JA.targ_lie(i_targ).jump(currJump).data = jointAngles_lie;
                    ...
                    if(isempty(modelLinkNames)) % only need to do this once
                    ...
                    if(numJoints==0)
                        ...
                        shldrElevJoints = find(contains(jointAngles_names,'shoulder_jElevation'));
                        elbowFlexJoints = find(contains(jointAngles_names,'elbow_jFlexion'));
                    end
                    
                    
                    % Determine if jump is represented well with 2D model
                    shldrSpeed = diff([JA.targ(i_targ).jump(currJump).data(:,shldrElevJoints), JA.targ(i_targ).jump(currJump).data(:,elbowFlexJoints)]);
%                     figure(1); clf; plot(shldrSpeed);
%                     ylim([-0.4 0.4]); grid on;
                    if(max(max(shldrSpeed(50:end,:))) > 0.1)
                        JA.bad2D(6*(i_set-1) + i_jump,i_targ) = max(max(shldrSpeed(50:end,:)));
                    end
...
%     end
    ...
    if(saveJAStruct)
        ...
        saveFilePath = [newdir 'results\JA_P' partNum '_2D.mat'];
