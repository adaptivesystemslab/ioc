...
targetDist = {'55','70','85'};
R_mat_frameNames = {'lshoulder_jElevation','rshoulder_jElevation','lhip_jFlexion','rhip_jFlexion'};
...
%                         [mdl_eul, trcMod_eul, initPos] = createJumpModel(trc, 1, initPosWindow, S2sc, calibPose, EKFCodePath);
    ...
    %                 jointAngles_lie = [];
                    ...
                    R_mat = [];
                    R_mat.shldr_L = zeros(size(mes_eul,1),3,3);
                    R_mat.shldr_R = zeros(size(mes_eul,1),3,3);
                    R_mat.hip_L = zeros(size(mes_eul,1),3,3);
                    R_mat.hip_R = zeros(size(mes_eul,1),3,3);
                    jointNames = {mdl_eul.joints.name};
    ...
    %                     jointAngles_lie = getLieModelJAs(mdl_lie);
                        ...
                        % Calculate shoulder rotation matricies (for Nature
                        % Sci Data Publication)
                        for i_R = 1:4
                            trNum = find(ismember(jointNames,R_mat_frameNames{i_R})==1);
                            R_0_pre = mdl_eul.joints(trNum).frame_in.t(1:3,1:3);
                            R_0_post = mdl_eul.joints(trNum+2).frame_out.t(1:3,1:3);
                            R_mat.(R_matNames{i_R})(i,:,:) = (R_0_pre')*R_0_post;
                        end
...
%                         saveFilePath = [newdir 'results\RESULTS_EKF_P' partNum '_' targetDist{i_targ} '_' num2str(i_set) '_' num2str(i_jump) '.mat'];
                        saveFilePath = [newdir 'results\RESULTS_EKF_P' partNum '_' targetDist{i_targ} '_' num2str(i_set) '_' num2str(i_jump) '_const_model_2D.mat'];
    ...
    %                          'ekf_state','lg_ekf_hatInvState');
                    ...
                    if(saveJointAngles)
                        ...
                        idcs = strfind(mydir,'\');
                        newdir = mydir(1:idcs(end)); 
                        saveFilePath = [newdir 'results\RESULTS_JA_P' partNum '_' targetDist{i_targ} '_' num2str(i_set) '_' num2str(i_jump) '_2D.mat'];
                        ...
                        save(saveFilePath,'jointAngles_names','jointAngles_eul',...
                            ...
                            'modelLinks','world2base','locationStart','locationLand','R_mat');
