...
%%%%%%%%%%%%%% perform shifting on each signal
    [shiftValues,shift_rec] = JAShiftValues(JA);
    ...
    for i_targ = 1:numel(targCurr)
        ...
        for j1 = 1:numel(plotJoints)
            ...
            for j2 = 1:numel(plotJoints{j1})
                ...
                title([plotNames(plotJoints{j1}(j2)) ', toe marker vel.']);
    %             ylim([ymin,ymax]);
                ylim([-150,150]);
                grid on;
                ...
                % and up to landing frame
                frame_st = JA.TOFrame(idx_LandFrame,i_targ) - 100;
                frame_end = JA.LandFrame(idx_LandFrame,i_targ);
                ...
                for i = 1:size(sig_dtw,2)-1 %use last jump as DTW reference
                    ...
                    if(shift > 0)
                        sig_dtw(:,i) = [zeros(shift,1); sig(1:end-shift,i)];
                    elseif(shift < 0)
                        sig_dtw(:,i) = [sig(1-shift:end,i); zeros(-shift,1)];
                    end
                end
                ...
                plot([JA.endFrame(idx_LandFrame,i_targ), JA.endFrame(idx_LandFrame,i_targ)],[-360,360],'k--');
                legend(jumpNames(1:numel(targCurr(i_targ).jump)));
                title([plotNames(plotJoints{j1}(j2)) ', signal error']);
                ylim([-150,150]);
                grid on;
