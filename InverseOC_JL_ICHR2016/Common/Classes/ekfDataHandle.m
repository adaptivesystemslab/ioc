       ...
       % load the data via parseCSV
        ...
        elseif isfield(ekfData, 'S1')
            ...
            for ind_ekfFieldNames = 1:length(ekfFieldNames)
                ...
                if ~strcmpi(currEkfFieldName(1), 'S')
                    % not a state. skip
                else
                    % attempt to parse the rest as a number
                    ekfFieldToParse = str2num(currEkfFieldName(2:end));
                    
                    if ~isempty(ekfFieldToParse) && ekfFieldToParse > totalStateCount
                        totalStateCount = ekfFieldToParse;
                    end
                end
            end
            
            % assume that the states divide evenly into 3: q, dq, ddq
            qStateCount = floor(totalStateCount/3);
            
            qArray = [];
            dqArray = [];
            ddqArray = [];
            for ind_stateCount = 1:qStateCount
