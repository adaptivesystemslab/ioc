classdef SVMThreeClassClassifier < AClassifier
...
%             searchArray = 10.^(-2:2:2);
            ...
            obj.modelParam.string = [obj.modelParam.string ' -b 1']; % generate sigmoid features
...
%             obj.plotSupportVectors(inputUnity, output, obj.SVMmodel);
            ...
            [trainingAlgLabel, error, decision_values] = svmpredict_mex(output, input, obj.SVMmodel, '-q');
            obj.trainingDV = decision_values;
            
            % determine the switching class boundaries
%             
...
%             prob = sigmoid_predict(obj, decision_values, obj.SVMmodel.ProbA, obj.SVMmodel.ProbB) - 0.5;       
            
            % perform class alteration based on distance to the hyperplane
            if ~isempty(obj.classThreeDV)
                decisionValue = decision_values;
%                  decisionValue = sigmoid_predict(obj, decision_values, obj.SVMmodel.ProbA, obj.SVMmodel.ProbB) - 0.5;       
                
                classThreePoints = find(abs(decisionValue) < obj.classThreeDV);
                out(classThreePoints) = 2;
            end
        end
...
%             fullEmission2 = 1/(1+exp(fApB));
        ...
        function adaptiveComponent(obj, trainingData)
            % pull in the training labels to adapt for class 3 conversion
            % distances
            classThreeLabels = find(trainingData.trainingLabel_class3 == 2);
            
            classThreeDVValues = obj.trainingDV(classThreeLabels);
%             classThreePVValues = sigmoid_predict(obj, obj.trainingDV(classThreeLabels), obj.SVMmodel.ProbA, obj.SVMmodel.ProbB) - 0.5;       
           
            meanThreeDistance = mean(abs(classThreeDVValues));
            
            obj.classThreeDV = meanThreeDistance;
            
            if 0
               figure;
%                plot(obj.trainingDV, 'rx');
               hold on
               plot(classThreeLabels, classThreeDVValues, 'bo');
               
               classThreeLabels = find(trainingData.trainingLabel_class3 == 1);
               classThreeDVValues = obj.trainingDV(classThreeLabels);
               plot(classThreeLabels, classThreeDVValues, 'rx');
               
               classThreeLabels = find(trainingData.trainingLabel_class3 == 0);
               classThreeDVValues = obj.trainingDV(classThreeLabels);
               plot(classThreeLabels, classThreeDVValues, 'go');
               plot([1 length(trainingData.trainingLabel_class3)], [meanThreeDistance meanThreeDistance], 'b');
               plot([1 length(trainingData.trainingLabel_class3)], [-meanThreeDistance -meanThreeDistance], 'b');
            end
        end
