 % assemble the final string over the various files
    ...
    for ind = 1:length(matFilesToLoad)
        ...
        if strcmpi( currLoadPackage.run_mode, 'sim')
            fprintf('    Loading sim data, bypassing MAT file labels on window length\n');
            currIndRange(1) = 1;
            currIndRange(2) = max(max(currLoadPackage.indToUse_window));
        elseif ind < length(matFilesToLoad)
