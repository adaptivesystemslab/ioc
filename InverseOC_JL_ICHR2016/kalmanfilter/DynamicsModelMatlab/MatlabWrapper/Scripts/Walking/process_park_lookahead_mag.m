...
%CCW Circle
%imu_data_time_stamp = '_2019_06_25_11_27_45';
%Static in 
...
m = [31.0 -7.0 -131.0 ]';
%m = [-0.3131   -0.1226   -0.9450]';
m = m/norm(m);
%m = rotz(pi/4-deg2rad(20))*m;
m = rotz(pi+deg2rad(60))*m;
mdl_imu.m = m;
...
for i=1:numel(imu_attachement)
    ...
    sens.addDecorator('magnetometer');
    T = eye(4);
...
for i=1:numel(imu_names)
    ...
    gyro_bias_2 = mean(imu.gyroscopeCalibrated(7984:10313,:));
    imu.gyroscopeCalibrated = imu.gyroscopeCalibrated - repmat(gyro_bias_2,size(imu.gyroscopeCalibrated,1),1);
    
    
    
    %NOTE YOU HAVE TO RUN MAG_TEST before TO GET THE CALIB VALUES
    imu.magnetometerCalibrated = imu.magnetometerUncalibrated - ...
        repmat(mag_calib(i).bias,size(imu.magnetometerUncalibrated,1),1);
    imu.magnetometerCalibrated = imu.magnetometerCalibrated./repmat(mag_calib(i).scale,size(imu.magnetometerCalibrated,1),1);
    %Now we have to apply the rotations
    imu.magnetometerCalibrated = (rotz(-pi/2)*rotx(pi)*rotx(pi)*rotz(-pi/2)*imu.magnetometerCalibrated')';
    
    bad_mag_indxs = find(sqrt(sum(imu.magnetometerCalibrated.^2,2))>1.5);
    imu.magnetometerCalibrated(bad_mag_indxs,:) = imu.magnetometerCalibrated(bad_mag_indxs-1,:);
    
    
    
    %Apply GHA rotation to everything
    imu.applyRotation(R);
    Rs(:,:,i) = R;
    imus(i) = imu;
...
for i = 1:numel(imus)
    ...
    mag = imus(i).magnetometerCalibrated(1:min_samples,:);
    
    mes_imu = [mes_imu gyro accel mag];
end
...
%Set the first revolute joints to high noise so MAG will get initial
%orientation
ekf_lc.covariance(4:6,4:6) = eye(3)*1;
                                        %GYRO           ACCEL    MAG
ekf_lc.observation_noise = diag(repmat([0.01 0.01 0.01 0.1 0.1 0.1 0.00001 0.00001 0.00001],1,num_imu));
...
mag_w = mag_no_z;
for i=1:size(mag_ub_scale_rot,1)
    mag_w(i,:) = Rs_world_imu(:,:,i)*mag_ub_scale_rot(i,:)';
