%% Walking in the park using alternating constrants with magnetometer
%At each timestep we use the previously estimated world z in pelvis IMU
%frame to subtract off any magnetic field in gravity direction then the
%result is rotated using the inverse of the first two joints to align the
%magnetic vector along world xy plane. Finally the magnetometer sensor is
%attached after the first joint which is in world Z
...
mdlFilepath = '..\..\Models\avatar_v04_p_zyx_bothLegs_zyx_hip.xml';
imu_mdlFilepath = '..\..\Models\avatar_v04_p_zyx_bothLegs_zyx_hip.xml';

%Straight Line RD Length of Box
%imu_data_time_stamp = '_2019_06_25_11_21_46';
%CCW Square
imu_data_time_stamp = '_2019_06_25_11_25_10';
%Static in 
imu_gha_static_time_stamp = '_2019_06_25_10_43_20';
imu_gha_dynamic_RL_time_stamp = '_2019_06_25_10_44_25';
imu_gha_dynamic_LL_time_stamp = '_2019_06_25_10_45_11';
imu_gha_dynamic_TF_time_stamp = '_2019_06_25_10_46_09';
...
%Set default Magnetic field to align model with facing along field 
%m = [31.0 -7.0 -131.0 ]';
m = [1 0 0];
m = m/norm(m);
mdl_imu.m = m;
...
for i=1:numel(imu_attachement)
    ...
    Rframe_imu = Rpelvis_frame'*roty(pi);
...
%% Attach Magnetometer to Pelvis 
mag_sens = SensorCore('mag');
mag_sens.addDecorator('magnetometer');
post_pelvis_rotz = mdl_imu.getFrameByName('post:b_pelvis:rotateX');
T = eye(4);
T(1:3,4) = [0.2 0 0]';
mdl_imu.addSensor(mag_sens,post_pelvis_rotz.name,T);


%% Attach YAW sensors to each leg 
b = mdl_imu.getFrameByName('post:b_pelvis');
...
num_mag = sum(contains({mdl_imu.sensors.type},'magnetometer'));
...
mag_ub_scale_rot_smooth = lowpass(mag_ub_scale_rot,10,imu(1).samplingRate);
%mes_imu = [mes_imu mag_ub_scale_rot_smooth];
mes_imu = [mes_imu mag_ub_scale_rot_smooth];
...
%ekf_lc.observation_noise(4:6,4:6)  = eye(3)*1;

%Mag Noise
ekf_lc.observation_noise = blkdiag(ekf_lc.observation_noise,diag(repmat([1000 1000 1000],1,num_mag)));
...
%Initial Covariance
ekf_lc.covariance = ekf_lc.process_noise*100;
...
%mag no z
mag_no_z_all = zeros(size(mes_imu,1),3);
...
for i=start_indx:start_indx+horizon-1
    ...
    %Compute mag-z in predicted state and put it into measurements
    mag_mes = z_myaw(end-2:end)';
    mdl_imu.position = ekf_rc.state(1:dof);
    mdl_imu.forwardPosition();
    z_imu = mdl_imu.sensors(1).transform(3,1:3)';
    mag_z = dot(mag_mes,z_imu)*z_imu;
    mag_no_z = z_myaw(end-2:end) - mag_z;
    mag_no_z = mag_no_z/norm(mag_no_z);
    mag_no_z_rot = (post_pelvis_rotz.t(1:3,1:3)'*mdl_imu.sensors(1).transform(1:3,1:3))*mag_no_z;
    z_myaw(end-2:end) = mag_no_z_rot;
    z(1:numel(z_myaw)) = z_myaw;
...
for i=start_indx+1:size(mes_imu,1)
    ...
    if cur_active == 'r'
        ...
        for indx = 1:size(z_hor,1)
            ...
            %Compute mag-z in predicted state and put it into measurements
            mag_mes = z_myaw(end-2:end)';
            mdl_imu.position = ekf_lc.state(1:dof);
            mdl_imu.forwardPosition();
            z_imu = mdl_imu.sensors(1).transform(3,1:3)';
            mag_z = dot(mag_mes,z_imu)*z_imu;
            mag_no_z = z_myaw(end-2:end) - mag_z;
...
mag_sens = SensorCore('mag');
mag_sens.addDecorator('magnetometer');
pelvis_frame = mdl_imu.getFrameByName('post:b_pelvis:rotateX');
...
mag_no_z_rot = mag_ub_scale_rot;
for i=1:size(mag_ub_scale_rot,1)
    mdl_imu.position = ekf_state(i,1:dof);
    mdl_imu.forwardPosition();
    z_imu = mdl_imu.sensors(1).transform(3,1:3)';
    mag_z_i = dot(mag_mes,z_imu)*z_imu;
    mag_no_z_i =  mag_ub_scale_rot(i,:)'- mag_z_i;
    mag_no_z_i = mag_no_z_i/norm(mag_no_z_i);
    mag_no_z_rot_i = (post_pelvis_rotz.t(1:3,1:3)'*mdl_imu.sensors(1).transform(1:3,1:3))*mag_no_z_i;    
    mag_no_z_rot(i,:) = mag_no_z_rot_i;
end
...
for i=1:size(mag_ub_scale_rot_smooth,1)
    z_imu(i,:) = mdl_imu.sensors(1).transform(3,1:3);
    mag_z(i,:) = dot(mag_ub_scale_rot_smooth(i,:),z_imu(i,:))*z_imu(i,:);
end
mag_no_z = mag_ub_scale_rot_smooth - mag_z;
mag_no_z = mag_no_z./repmat(sqrt(sum(mag_no_z.^2,2)),1,3);
