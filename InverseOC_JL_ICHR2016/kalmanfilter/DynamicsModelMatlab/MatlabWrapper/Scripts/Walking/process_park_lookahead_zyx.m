...
%Here our floating base has ZYX rotation insteaf of XYZ
...
%Magnetometer rotated to pelvis frame
mag_no_z_rot = (roty(-pi/2)*mag_no_z')';

pelvis_frame = mdl_imu.getFrameByName('post:b_pelvis:rotateX');


%Rotate back using the joints as well
for i=1:size(mag_ub_scale_rot,1)
    mdl_imu.position = ekf_state(i,1:dof);
    mdl_imu.forwardPosition();
    mag_no_z_rot(i,:) = (pelvis_frame.t(1:3,1:3)'*mdl_imu.sensors(1).transform(1:3,1:3))*mag_no_z(i,:)';
end
