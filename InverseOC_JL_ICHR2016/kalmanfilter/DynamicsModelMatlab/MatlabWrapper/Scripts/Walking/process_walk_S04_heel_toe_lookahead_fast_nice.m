...
ekf_rh_state_diff = zeros(size(mes_imu,1),ekf_lhc.sizeX);
ekf_lh_state_diff = zeros(size(mes_imu,1),ekf_lhc.sizeX);
ekf_rt_state_diff = zeros(size(mes_imu,1),ekf_lhc.sizeX);
ekf_lt_state_diff = zeros(size(mes_imu,1),ekf_lhc.sizeX);
...
horizon = 5;       %How far we look ahead
state_diff_thresh = 0.2;
%Saved States
...
for i=start_indx+1:size(mes_imu,1)
    ...
    if strcmp(cur_active,'rh')
        ...
        for indx = 1:size(z_hor,1)
            ...
            %If the state is changing too much to satisty the constraint
            %move on
            if norm(ekf_rt_state_diff(i+indx-1,1:end/3)) > state_diff_thresh || cur_count < 10
                hor_Es_rt(:) = inf;
                break;
            end
    ...
    elseif strcmp(cur_active,'lh')
        ...
        for indx = 1:size(z_hor,1)
            ...
            %If the state is changing too much to satisty the constraint
            %move on
            if norm(ekf_lt_state_diff(i+indx-1,1:end/3)) > state_diff_thresh || cur_count < 10
                hor_Es_lt(:) = inf;
                break;
            end
    ...
    elseif strcmp(cur_active,'rt')
        ...
        for indx = 1:size(z_hor,1)
            ...
            %If the state is changing too much to satisty the constraint
            %move on
            if norm(ekf_lh_state_diff(i+indx-1,1:end/3)) > state_diff_thresh || cur_count < 10
                hor_Es_lh(:) = inf;
                break;
            end
    ...
    elseif strcmp(cur_active,'lt')
        ...
        for indx = 1:size(z_hor,1)
            ...
            %If the state is changing too much to satisty the constraint
            %move on
            if norm(ekf_rh_state_diff(i+indx-1,1:end/3)) > state_diff_thresh || cur_count < 10
                hor_Es_rh(:) = inf;
                break;
            end
    ...
    elseif strcmp(cur_active,'rh') && Ert < Erh
        ...
        %    cur_active = 'lh';
        %    cur_count = 0;
    elseif strcmp(cur_active,'rt') && Elh < Ert
        cur_active = 'lh';
        cur_count = 0;
