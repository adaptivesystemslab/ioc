...
imu_data_time_stamp = '_2019_06_07_11_29_54';
...
ekf_state_orig = zeros(size(mes_imu,1),ekf_lc.sizeX);

ekf_r_state_diff = zeros(size(mes_imu,1),ekf_lc.sizeX);
ekf_l_state_diff = zeros(size(mes_imu,1),ekf_lc.sizeX);
...
%The errors
Er_all = zeros(size(mes_imu,1),1);
El_all = zeros(size(mes_imu,1),1);
...
for i=start_indx+1:size(mes_imu,1)
    ...
    if cur_active == 'r'
        ...
        ekf_r_state_diff(i+horizon-1,:) = ...
            ekf_rc.state-ekf_rc.state_orig;
        ...
        for indx = 1:size(z_hor,1)
            ...
            %Run Left Constrained and populate horizon vars
            ekf_lc.run_iteration(dt,z);
            ekf_l_state_diff(i+indx-1,:) = ...
            ekf_lc.state-ekf_lc.state_orig;
    ...
    if cur_active == 'l' && Er < El && norm(ekf_r_state_diff(i,1:end/3*2)) < 0.5 && cur_count > 20
        %We think left leg is swinging, but left leg error is larger than
        %right by a lot, switch constraint to right leg
        cur_active = 'r';
        cur_count = 0;
    elseif cur_active == 'r' && El < Er && norm(ekf_l_state_diff(i,1:end/3*2)) < 0.5 && cur_count > 20
        cur_active = 'l';
        cur_count = 0;
    end
