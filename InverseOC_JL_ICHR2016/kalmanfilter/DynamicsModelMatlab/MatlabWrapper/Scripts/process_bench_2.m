...
bar_trc = parseTRC('C:\aslab\data\bench2\bench_1_joined_smoothed-bAR.trc');
bod_trc = parseTRC('C:\aslab\data\bench2\bench_1_joined_smoothed-SSA_Pilot.trc');
...
marker_attachement_rarm = ...
    ...
    {'body_rwrist_rhand','WRIST_R_IMU_T','WRIST_R_IMU_L','WRIST_R_IMU_R'};
    };
...
marker_attachement_larm = ...
    ...
    {'body_lwrist_lhand','WRIST_L_IMU_T','WRIST_L_IMU_L','WRIST_L_IMU_R'};
    };
...
time_stamp = '2019_02_26_12_12_31';
...
%Add a constraint that forces end effector positions and rotations to be
%the same at the middle of the bar. Note after model building the rotations
%of end effectors are already aligned so we just need to match the
%positions
bar_mid = (l_bar_end+r_bar_end)/2;
%Translation from right hand to bar middle in world frame
r_hand_to_bar_mid = bar_mid - r_hand;
%Translation from right hand to bar middle in r_hand frame
t_r_hand_to_bar_mid = R0_rh'*r_hand_to_bar_mid';

%Translation from left hand to bar middle in world frame
l_hand_to_bar_mid = bar_mid - l_hand;
%Translation from left hand to bar middle in r_hand frame
t_l_hand_to_bar_mid = R0_rh'*l_hand_to_bar_mid';

%So now we can set up the constraint transforms
%C1 is left and C2 is right
T_C1 = eye(4);
T_C1(1:3,4) = t_l_hand_to_bar_mid;
T_C2 = eye(4);
T_C2(1:3,4) = t_r_hand_to_bar_mid;

C1 = EKF_Constraint_BF('C1',mdl_larm,'frame_lhand_end',T_C1,mdl_rarm,'frame_rhand_end',T_C2);
C1.type(:) = false;C1.type = logical([1 1 1 1 1 1]');
