...
bar_trc = parseTRC('C:\aslab\data\bench\lyingdown_sliding_1_interpolated_smoothed-bAR.trc');
bod_trc = parseTRC('C:\aslab\data\bench\lyingdown_sliding_1_interpolated_smoothed-SSA_Pilot.trc');
...
t_wri_r_bar_end = r_bar_end - r_wrist_or;
...
%Put end of wrist between the two left bar markers 
left_u = mean(bar_trc.data.LEFT_U(mean_indxs,:));
left_l = mean(bar_trc.data.LEFT_L(mean_indxs,:));
l_wrist_en = (left_u + left_l)' / 2;
t_indx = find(contains({mdl_larm.transforms.name},'fixed_lwrist_2'),1);
trans = mdl_larm.transforms(t_indx);
f_in = trans.frame_in;
f_out = trans.frame_out;
t_0 = l_wrist_en - f_in.t(1:3,4);
t_f = f_in.t(1:3,1:3)'*t_0;
trans.t(1:3,4) = t_f;
mdl_larm.forwardPosition();

%Finally make the actual bar and align ee with world 
right_u = mean(bar_trc.data.RIGHT_U(mean_indxs,:));
right_l = mean(bar_trc.data.RIGHT_L(mean_indxs,:));
bar_en = (right_u + right_l)/2;
...
t_bar_0 = bar_en' - l_wrist_en;
t_bar_f = f_in.t(1:3,1:3)'*t_bar_0;
trans.t(1:3,1:3) = f_in.t(1:3,1:3)';
trans.t(1:3,4) = t_bar_f;
mdl_larm.forwardPosition();
...
%Finally make the end effector the end of the bar and aligned with world
right_u = mean(bar_trc.data.RIGHT_U(mean_indxs,:));
right_l = mean(bar_trc.data.RIGHT_L(mean_indxs,:));
r_bar_end = (right_u+right_l)/2;
...
time_stamp = '2019_02_07_13_59_14';
...
ekf.observation_noise = diag(repmat([0.1 0.1 0.1 10 10 10],1,5));
...
C1.type(:) = false;C1.type = logical([0 1 0 1 0 1]');
