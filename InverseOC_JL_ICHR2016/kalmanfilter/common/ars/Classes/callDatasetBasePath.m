function pathToRawData = callDatasetBasePath(specStruct)
    ...
    basepath = 'D:\aslab\data\'; % if all your data is in one location, just update the basepath
    
    % update filepath to local installations
    switch lower(specStruct.datasetName) % move all to smallcaps
        case 'healthy1'
            pathToRawData = [basepath 'Lowerbody_healthy1_2011\'];

        case 'healthy2'
            pathToRawData = [basepath 'Lowerbody_healthy2_2013\']; % path to the raw data
            
        case 'stjoseph'
            pathToRawData = [basepath  'Lowerbody_StJoseph1_2013\'];
            
        case 'tri'
            pathToRawData = [basepath  'Lowerbody_TRI1_2012\'];
    end
