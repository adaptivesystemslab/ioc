...
%Reads a TRC motion capture file into a structure
    ...
    if length(fields) == 2 && size(M, 2) > 2
        % loading an unnamed marker file. fill the headers with placeholders
        numUnnamed = floor((size(M, 2) - 2)/3); % remove the frame and time row
        for i = 1:numUnnamed
            fields{end+1} = ['U' pad(num2str(i), 8, 'left', '0')];
        end
    end
