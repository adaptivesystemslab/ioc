function [q, dq, ddq, t] = quinticSpline(y, dy, ddy, x, dt)  
...
%   dt - the delta time of the data
    ...
    for ind_y = 1:size(y, 1)-1
        indToUse = x(ind_y):x(ind_y+1);
        t_curr = t(x(ind_y):x(ind_y+1));
        ...
        for ind_dof = 1:size(y, 2)
            ...
            q(indToUse, ind_dof) =     polyval(sp_qd0, t_curr);
            dq(indToUse, ind_dof) =    polyval(sp_qd1, t_curr);
            ddq(indToUse, ind_dof) =   polyval(sp_qd2, t_curr);
        end
    end
end
