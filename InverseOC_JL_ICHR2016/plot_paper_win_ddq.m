...
if exist('h14_ddq', 'var') && h14_ddq > 0
    figure(h14_ddq);
else
    h14_ddq = figure;
end
...
plot(feature_full.t, ddq_opt_plot_merge);
plot(t_recon_plot_merge, ddq_recon_plot_merge, '.', 'LineWidth', 1);
title(['Trajectory and Reconstruction (RMSE (blended, windowed) : ' ...
    num2str(rmse_report.blended_rmse_belowThreshold_ddq) ', ' num2str(rmse_report.windowed_rmse_belowThreshold_ddq_mean) ...
    ' \pm ' num2str(rmse_report.windowed_rmse_belowThreshold_ddq_std) ')']);
