...
if exist('h7', 'var') && h7 > 0
    figure(h7);
else
    h7 = figure('position', [  1376         111        1906         998]);
end



ax(1) = subplot(2, 2, 1); hold on
title('Recovered contribution ratio (sliding)');
bar(t_rmse(uniqueInd), ratioRMSE_plot(uniqueInd, :), 'stacked'); 

% plot(t_rmse(pts_deny), zeros(size(find(pts_deny)))-0.05, 'rx');
plot(t_rmse(pts_allow), zeros(size(find(pts_allow)))  - 0, 'gx');
plot(t_rmse(pts_allow), zeros(size(find(pts_allow)))  + 100, 'gx');
...
title('Recovered contribution weights (avg)');
...
title(['Const (b) resnorm (mu=' num2str(meanconstResnrom) '), and threshold (k)']);
...
plot(t_rmse(pts_underConstThres), zeros(size(find(pts_underConstThres)))  -0, 'gx');
