...
%             input = [THETA, RHO, Z];
        ...
        function error = train(obj,input,output)
            ...
            SVMmodel = svmtrain_mex(output, input, obj.modelParam.string);
            [trainingAlgLabel, error, ~] = svmpredict_mex(output, input, SVMmodel, '-q');
%             error = 1-error(1)/100;
            acc1 = sum(output == trainingAlgLabel)/length(trainingAlgLabel);
            
            % keep only the points that was correctly classified
            matchingPoints = output == trainingAlgLabel;
            outputMatching = output(matchingPoints);
            inputMatching = input(matchingPoints, :);
            obj.SVMmodel = svmtrain_mex(outputMatching, inputMatching, obj.modelParam.string);
            [trainingAlgLabelMatch, error, ~] = svmpredict_mex(outputMatching, inputMatching, obj.SVMmodel, '-q');
            acc2 = sum(outputMatching == trainingAlgLabelMatch)/length(trainingAlgLabelMatch);
