        ...
        function init(obj,classifierSelect)
            ...
            switch classifierSelect.adaptMethod
                ...
                case {'clusterDR'}
                    ...
                    param.percentageOriginalDataToKeepFirstReplacement = str2num(classifierSelect.adaptParam{9})/100; 
                    
                    % percentage of the original data to keep after the first replacement
                    param.percentageOriginalDataToKeepSubsequentReplacement = str2num(classifierSelect.adaptParam{10})/100;
                    
                     % distance from the core to keep in percentage
                    param.percentageDistanceFromCoreToKeep = str2num(classifierSelect.adaptParam{11})/100;
...
%                     prob_inc = [];
%                 end
%                 trainingDataParse = clusterDataGateway(extra, obj.incsvmParam{1}, 0);

%                 % recall that we want to load all the data
                if ~isempty(extra.testingLabel)
                    switch obj.clusterDR.dataFormatName
                        case 'PC'
                            %                     inputCluster = extraInput.testingData(ind_range, :);
                            inputRange = 1:length(extra.testingLabel);
                            trainingDataParse = clusterDataGatewayTesting(extra, obj.incsvmParam{1}, inputRange);
                        ...
                        case 'PP'
                            ...
                            inputRange = (extra.settings.dimStrack+1):(length(extra.label)-extra.settings.dimStrack);
                            trainingDataParse = clusterDataGatewayTesting(extra, obj.incsvmParam{1}, inputRange);
                    end
                else
                    % training, so we only want the same points as the
                    % training
                    trainingDataParse = clusterDataGatewayTraining(extra, obj.incsvmParam{1}, 0);
                end

                [out_inc, prob_inc] = obj.clusterDR.classify(trainingDataParse.input);
...
%                 trainingDataParse = clusterDataGateway(extra, obj.incsvmParam{1}, 0);
                ...
                obj.clusterDR.classifyAndPlot(trainingDataParse.input, trainingDataParse);
            end
