...
function [h, rsq_mean, rsq_std] = plot_weightReps(h, outputPath, currInstName, feature_full, avgWeightArray_belowThres, ...
    resnromAll_lsqlin_const_minRMSE_array_belowThres, lambda_name, ...
    segmentInfo, plotGroups, runMode)
ax = [];
lambda_count = size(avgWeightArray_belowThres, 2);
...
for ind_costf = 1:lambda_count
    ...
    plot(x_val, f1, 'b-');
    title([lambda_name{ind_costf} ' weight mean R^2 = ' num2str(rmse_Rsq1)]);
    
    subplot(212);
    plot(x_val, y2_val, 'o'); hold on
    plot(x_val, f2, 'b-');
    title([lambda_name{ind_costf} ' weight stddev R^2 = ' num2str(rmse_Rsq2)]);
