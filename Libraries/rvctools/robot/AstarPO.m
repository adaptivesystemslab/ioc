...
classdef AstarPO < Navigation
    ...
    methods % start of public methods
        ...
        function as = AstarPO(world, varargin)
            %AstarPO.AstarPO A*-PO constructor
            %
            % AS = AstarPO(MAP, OPTIONS) is a A* navigation object, and MAP is an
        ...
        function reset(as)
            %AstarPO.reset Reset the planner
            %
        ...
        function s = char(as)
            %AstarPO.char Convert navigation object to string
            %
        ...
        function plot(as, varargin)
            %AstarPO.plot Visualize navigation environment
            %
       ...
       % invoked by Navigation.step
        ...
        function plan(as, goal, N)
            %AstarPO.plan Prep the grid for planning.
            %
        ...
        %AstarPO.path Find a path between two points
        %
        ...
        function navigate_init(as, start)
            ...
            while ~isempty(as.openlist) % L4
                ...
                [idx] = paretofront(normc(as.openlist(2:size(as.openlist,1),:)')); % w/ normalization
                front = as.openlist(:,idx);
                [~,col] = min(front(as.tie+1,:)); % as.tie specifies the row (cost_g) for tiebreaker
                X = front(1,col); % L5; minimum state on the Pareto front
                as.DELETE(X); % L6; remove state from the openlist
                
                as.niter = as.niter + 1;
                as.priority = as.cost_g; % updates priority cost layer
        ...
        %AstarPO.cost_get Get the specified cost layer
            layer = as.cost_02;
        end
        
        function c = heurstic_get(as)
        %AstarPO.heuristice_get Get the current heuristic map
        %
        ...
        %AstarPO.costmap_get Get the current costmap
        %
        ...
        %AstarPO.costmap_set Set the current costmap
        %
        ...
        %AstarPO.costmap_modify Modify cost map
        %
