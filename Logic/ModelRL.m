    ...
    methods
        ...
        function traj = filtData(obj, time, dt, qRaw, trialInfo)
            ...
            if ~isempty(trialInfo.runInds)
                if trialInfo.runInds(1) > 0 && trialInfo.runInds(2) > 0
                    startInd = trialInfo.runInds(1);
                    endInd = trialInfo.runInds(2);
                else
                    startInd = length(time)-abs(trialInfo.runInds(1));
                    endInd = length(time)-abs(trialInfo.runInds(2));
                end
